...
Everything runs headless from a terminal — no Visual Studio involved. Earlier docs
(`SHIPPING.md`) describe a VS right-click "Publish" flow; these projects have no
`.pubxml` publish profiles, so that flow just errors with "ModPublisherCommand is
not set". This file is the authoritative workflow, verified end-to-end 2026-07-16
(Building Reviver's first publish and a listing-only update both ran through it).
...
- Cemetery Overflow Manager: unpublished
